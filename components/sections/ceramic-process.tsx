...
import { ceramicProcessSteps } from "@/data/ceramic-page";
import { EASE, viewport } from "@/lib/motion";

export function CeramicProcess() {
  return (
      ...
      <Container className="grid gap-14 md:grid-cols-[1fr_1.4fr] md:gap-20">
        <div className="md:sticky md:top-28 md:self-start">
          <h2 className="text-balance text-[clamp(2.1rem,5vw,3.75rem)] font-bold uppercase leading-[0.98] tracking-tight text-ink">
            How it
            <br />
            <span className="text-ink-muted">works.</span>
          </h2>
          <p className="mt-8 max-w-sm text-base leading-relaxed text-ink-muted md:text-lg">
            Choose your coating, add the protection you need, and we&rsquo;ll
            handle the rest.
          </p>
        </div>

        <div className="border-t border-border">
          {ceramicProcessSteps.map((step, i) => (
            <motion.div
              key={step.number}
